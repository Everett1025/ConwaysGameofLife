...
// COUNTS THE SURROUNDING NUMBER OF LIVE CELLS AROUND G_DATA[X1 + Y1]
static inline unsigned int gol_countAliveCells(unsigned char *data, size_t x0, size_t x1, size_t x2, size_t y0, size_t y1, size_t y2)
{
...
void gol_iterateSerial(size_t iterations)
{
    ...
    for (i = 0; i < iterations; ++i)
    {
        for (y = 0; y < g_worldHeight; ++y)
        {
            ...
            {
                //SET VALUES OF X0 X1 X2 call countAliveCells and compute if g_resultsData[y1 + x1] is 0 or 1
                size_t x1 = x;
                size_t x0 = (x1 + g_worldWidth - 1) % g_worldWidth;
                size_t x2 = (x1 + 1) % g_worldWidth;

                // STORE THE NUMBER OF ALIVE CELLS SURROUNDING G_DATA[X1 + Y1]
                int aliveCount = gol_countAliveCells(g_data, x0, x1, x2, y0, y1, y2);

                // IF THE CURRENT CELL IS ALIVE
                if (g_data[y1 + x1] == 1)
                {
                    // UPDATE BOARD BASED OFF GAME REQUIREMENTS
                    if (aliveCount == 2 || aliveCount == 3)
                    {
                        g_resultData[y1 + x1] = 1;
                    }
                    else
                    {
                        g_resultData[y1 + x1] = 0;
                    }
                }
                else // CURRENT CELL IS DEAD
                {
                    // UPDATE BOARD BASED ON GAME REQUIREMENTS
                    if (aliveCount == 3)
                    {
                        g_resultData[y1 + x1] = 1;
                    }
        ...
        // (PASS IN ADDRESSES SO POINTERS ACTUALLY SWAP AND NOT THE VALUES THEY POINT TO)
